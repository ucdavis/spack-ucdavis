...
class Phyluce(PythonPackage):
    ...
    depends_on("python@2.7", type=("build", "run"), when="@:1.6.7")
    depends_on("python@3.10:", type=("build", "run"), when="@1.7.0:")
    
    depends_on("py-rich", type="run", when="@1.7.0:")
    depends_on("snakemake", type="run", when="@1.7.0:")
