...
class IntelOneapiCompilersClassic(Package, CompilerPackage):
        ...
        if "~envmods" not in self.spec:
            env.extend(
                EnvironmentModifications.from_sourcing_file(
                    self.prefix.env.join("vars.sh")
                )
            )

        env.set("CC", self.prefix.icc)
        env.set("CXX", self.prefix.icpc)
        env.set("F77", self.prefix.ifort)
        env.set("FC", self.prefix.ifort)
    ...
    def install(self, spec, prefix):
        ...
        self.symlink_dir(self.oneapi_compiler_prefix.env, prefix.env)
