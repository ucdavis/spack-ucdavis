...
class Soci(CMakePackage):
    ...
    variant("mysql", default=False, description="Build with MySQL support")
    variant("tests", default=False, description="Build Catch tests")
    #variant("db2", default=False, description="Build with DB2 support")
    #variant("firebird", default=False, description="Build with Firebird support")
    ...
    def cmake_args(self):
        args = [
            ...
            self.define_from_variant("WITH_MYSQL", "mysql"),
            self.define_from_variant("SOCI_TESTS", "tests")
        ]
