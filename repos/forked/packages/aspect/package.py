...
class Aspect(CMakePackage):
    ...
    patch("assert-semicolons.patch",
          sha256="0a958a8246f81d45d2d4933cdab10b3f08145f4542044755dfcb2deedbe546ec")

    def patch(self):
        filter_file(r'SET\(ASPECT_SOURCE_DIR\s+\$\{CMAKE_SOURCE_DIR\}\)',
                    f'SET(ASPECT_SOURCE_DIR "{self.prefix.source}")',
                    "CMakeLists.txt")
    ...
    def cmake_args(self):
        return [self.define_from_variant("ASPECT_USE_FP_EXCEPTIONS", "fpe"),
                self.define("ASPECT_SOURCE_DIR", self.prefix.source)]
